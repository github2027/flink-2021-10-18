...
public class Flink02_WaterMark_forBounded {
    public static void main(String[] args) throws Exception {
        ...
        //      @Override -----该方法每隔一段时间调用一次创建一个WaterMark (默认Watermark周期性生成)
        //	    public void onPeriodicEmit(WatermarkOutput output) {
        //		output.emitWatermark(new Watermark(maxTimestamp - outOfOrdernessMillis - 1));
        //	    }
        //      上述推出watermark为Long的最小值
        //      @Override -----该方法来一条数据调用一次创建一个waterMark
        //	    public void onEvent(T event, long eventTimestamp, WatermarkOutput output) {
        //		maxTimestamp = Math.max(maxTimestamp, eventTimestamp);
        //	    }
        //      Periodic And Event
        //      当短时间内数据量特别大，适合调用周期性生成的
        //      当长时间内数据量特别小，适合调用间歇性生成的
        //      WaterMark单调增长的原因
        //      maxTimestamp = Math.max(maxTimestamp, eventTimestamp) 如果当前事件的时间大于当前的最大时间，那么就会重新赋值给当前的最大时间，触发一次WaterMark
        //      周期性的周期时间间隔为200ms
        //      <p>If you set the characteristic to IngestionTime of EventTime this will set a default
        //	 * watermark update interval of 200 ms.
        //
